...
am0_traj = model.integrate(0, k0, 1.0, T=100, integrator='backward_euler')

am1_traj = model.integrate(0, k0, 1.0, T=100, integrator='trapezoidal_rule')

am2_traj = model.integrate(0, k0, 1.0, T=100, integrator='am2')

am3_traj = model.integrate(0, k0, 1.0, T=100, integrator='am3')
...
traj_error = model.plot_approximation_error(am0_traj, 
                                            analytic_trajectory, 
                                            log=True)[1]
traj_error.set_label('s=0')
traj_error.set_marker('o')
traj_error.set_linestyle('none')

# plot the ab2 approximation error
traj_error1 = model.plot_approximation_error(am1_traj, 
                                             analytic_trajectory, 
                                             log=True)[1]
traj_error1.set_label('s=1')
traj_error1.set_color(colors[1])
traj_error1.set_marker('o')
traj_error1.set_linestyle('none')

# plot the am2 approximation error
traj_error2 = model.plot_approximation_error(am2_traj, 
                                             analytic_trajectory, 
                                             log=True)[1]
traj_error2.set_label('s=2')
traj_error2.set_color(colors[2])
traj_error2.set_marker('o')
traj_error2.set_linestyle('none')

# plot the am3 approximation error
traj_error3 = model.plot_approximation_error(am3_traj, 
                                             analytic_trajectory, 
                                             log=True)[1]
traj_error3.set_label('s=3')
traj_error3.set_color(colors[3])
traj_error3.set_marker('o')
traj_error3.set_linestyle('none')

# plot the am4 approximation error
traj_error4 = model.plot_approximation_error(am4_traj, 
                                             analytic_trajectory, 
                                             log=True)[1]
traj_error4.set_label('s=4')
...
plt.title('Solow (1956) approximation error using\n' + 
          'Adams-Moulton methods, $h=1.0$', fontsize=20, family='serif')
plt.legend(loc='best', frameon=False, prop={'family':'serif'})
plt.savefig('graphics/solow-approximation-error-am-methods.pdf')
plt.show() 